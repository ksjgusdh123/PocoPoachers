...
public static class PacketBuilder
{
    public static ArraySegment<byte> Build(FlatBufferBuilder builder, PacketType type, int innerOffset)
    {
        ...
        if (totalSize > ushort.MaxValue)
            throw new InvalidOperationException(
                $"Packet too large: {totalSize} bytes (type={type}).");
    ...
    public static void Send(ClientSession session, FlatBufferBuilder builder, PacketType type, int innerOffset)
    {
        session.Send(Build(builder, type, innerOffset));
    }

    public static void Broadcast(IEnumerable<ClientSession> sessions, FlatBufferBuilder builder, PacketType type, int innerOffset)
    {
        ArraySegment<byte> segment = Build(builder, type, innerOffset);
