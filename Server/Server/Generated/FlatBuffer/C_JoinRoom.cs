// <auto-generated>
//  automatically generated by the FlatBuffers compiler, do not modify
// </auto-generated>

using global::System;
using global::System.Collections.Generic;
using global::Google.FlatBuffers;

public struct C_JoinRoom : IFlatbufferObject
{
  private Table __p;
  public ByteBuffer ByteBuffer { get { return __p.bb; } }
  public static void ValidateVersion() { FlatBufferConstants.FLATBUFFERS_24_3_25(); }
  public static C_JoinRoom GetRootAsC_JoinRoom(ByteBuffer _bb) { return GetRootAsC_JoinRoom(_bb, new C_JoinRoom()); }
  public static C_JoinRoom GetRootAsC_JoinRoom(ByteBuffer _bb, C_JoinRoom obj) { return (obj.__assign(_bb.GetInt(_bb.Position) + _bb.Position, _bb)); }
  public void __init(int _i, ByteBuffer _bb) { __p = new Table(_i, _bb); }
  public C_JoinRoom __assign(int _i, ByteBuffer _bb) { __init(_i, _bb); return this; }

  public string SessionCode { get { int o = __p.__offset(4); return o != 0 ? __p.__string(o + __p.bb_pos) : null; } }
#if ENABLE_SPAN_T
  public Span<byte> GetSessionCodeBytes() { return __p.__vector_as_span<byte>(4, 1); }
#else
  public ArraySegment<byte>? GetSessionCodeBytes() { return __p.__vector_as_arraysegment(4); }
#endif
  public byte[] GetSessionCodeArray() { return __p.__vector_as_array<byte>(4); }
  public NetInfo? MyInfo { get { int o = __p.__offset(6); return o != 0 ? (NetInfo?)(new NetInfo()).__assign(__p.__indirect(o + __p.bb_pos), __p.bb) : null; } }

  public static Offset<C_JoinRoom> CreateC_JoinRoom(FlatBufferBuilder builder,
      StringOffset session_codeOffset = default(StringOffset),
      Offset<NetInfo> my_infoOffset = default(Offset<NetInfo>)) {
    builder.StartTable(2);
    C_JoinRoom.AddMyInfo(builder, my_infoOffset);
    C_JoinRoom.AddSessionCode(builder, session_codeOffset);
    return C_JoinRoom.EndC_JoinRoom(builder);
  }

  public static void StartC_JoinRoom(FlatBufferBuilder builder) { builder.StartTable(2); }
  public static void AddSessionCode(FlatBufferBuilder builder, StringOffset sessionCodeOffset) { builder.AddOffset(0, sessionCodeOffset.Value, 0); }
  public static void AddMyInfo(FlatBufferBuilder builder, Offset<NetInfo> myInfoOffset) { builder.AddOffset(1, myInfoOffset.Value, 0); }
  public static Offset<C_JoinRoom> EndC_JoinRoom(FlatBufferBuilder builder) {
    int o = builder.EndTable();
    return new Offset<C_JoinRoom>(o);
  }
  public C_JoinRoomT UnPack() {
    var _o = new C_JoinRoomT();
    this.UnPackTo(_o);
    return _o;
  }
  public void UnPackTo(C_JoinRoomT _o) {
    _o.SessionCode = this.SessionCode;
    _o.MyInfo = this.MyInfo.HasValue ? this.MyInfo.Value.UnPack() : null;
  }
  public static Offset<C_JoinRoom> Pack(FlatBufferBuilder builder, C_JoinRoomT _o) {
    if (_o == null) return default(Offset<C_JoinRoom>);
    var _session_code = _o.SessionCode == null ? default(StringOffset) : builder.CreateString(_o.SessionCode);
    var _my_info = _o.MyInfo == null ? default(Offset<NetInfo>) : NetInfo.Pack(builder, _o.MyInfo);
    return CreateC_JoinRoom(
      builder,
      _session_code,
      _my_info);
  }
}

public class C_JoinRoomT
{
  public string SessionCode { get; set; }
  public NetInfoT MyInfo { get; set; }

  public C_JoinRoomT() {
    this.SessionCode = null;
    this.MyInfo = null;
  }
}


static public class C_JoinRoomVerify
{
  static public bool Verify(Google.FlatBuffers.Verifier verifier, uint tablePos)
  {
    return verifier.VerifyTableStart(tablePos)
      && verifier.VerifyString(tablePos, 4 /*SessionCode*/, false)
      && verifier.VerifyTable(tablePos, 6 /*MyInfo*/, NetInfoVerify.Verify, false)
      && verifier.VerifyTableEnd(tablePos);
  }
}
