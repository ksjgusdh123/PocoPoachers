...
public abstract class EquipableController : MonoBehaviour
{
    ...
    public abstract void Equip(ItemData data, int slotIndex, int uid);
    ...
    // 해당 슬롯에 장착된 아이템 인스턴스 uid (없으면 0) — 해제 시 인벤토리 슬롯에 되돌리는 데 사용
    public abstract int GetEquippedUid(int slotIndex);
