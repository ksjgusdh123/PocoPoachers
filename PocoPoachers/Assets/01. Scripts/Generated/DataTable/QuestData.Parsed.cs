// 손으로 작성한 partial 확장 — Tools/Generator/Tables로 재생성해도 지워지지 않는다 (GunStatData.Clone.cs와 동일한 방식).
// goal_item_ids/goal_item_counts, reward_item_ids/reward_item_counts는 "801|802" 형태의 '|' 구분 문자열이다.
...
{
    public IReadOnlyList<(int itemId, int count)> GoalItems => ParsePairs(goal_item_ids, goal_item_counts);

    public IReadOnlyList<(int itemId, int count)> RewardItems => ParsePairs(reward_item_ids, reward_item_counts);
